...
class HasilPerhitunganController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = HasilPerhitungan::with(['bahanBakar', 'transportasi', 'biaya'])
        ...
        if ($request->filled('search')) {
            $query->where('metode', 'like', '%' . $request->search . '%');
        }

        $perhitungan = $query->latest()->paginate(10)->withQueryString();
        $totalEmisi = $query->sum('hasil_emisi');

        return view('backend.pages.perhitungan.index', compact(
            'bahanBakar', 'jenis', 'biayaList', 'perhitungan', 'totalEmisi'
        ));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('backend.pages.perhitungan.create', compact(
            'bahanBakar', 'jenis', 'jenisKendaraan', 'kategori', 'metode'
        ));
    }
    ...
    public function edit($id)
    {
        ...
        return view('backend.pages.perhitungan.edit', compact(
            'perhitungan', 'kategori', 'metode', 'bahanBakar', 'jenis', 'biayaList'
        ));
    }
